...
\hypersetup{
  pdftitle={Agricultural Markets and Prices},
...
\addcontentsline{toc}{chapter}{Market Fundamentals}
...
\addcontentsline{toc}{chapter}{Price Relationships}
...
\hypertarget{storage-and-seasonality-of-agricultural-prices}{%
\section{Storage and Seasonality of Agricultural Prices}\label{storage-and-seasonality-of-agricultural-prices}}

Seasonal price behaviour is a systematic pattern that occurs within a year, and repeats across the years. For food and agricultural commodities, the main source of seasonality is the supply-side effects. Although, demand-side effects are also occasionally evident (e.g., high demand for turkey meat in the U.S. around the Thanksgiving period in November).

For an annually produced storable commodity (e.g., grains) supply is given by the production in the current period, plus the beginning inventory (i.e., the leftover stocks from previous marketing year), and minus the ending inventory. In a perfectly competitive market, prices will be lowest just after the harvest, and rise at the rate of \emph{cost of storage} per unit of time.

The decision about the inventory storage depends on the relationship of the expected price for delivery in the future, relative to the current price. Storage has important price--stabilizing role over the marketing year, as it creates a pricing buffer by shifting stocks forward. The driving force in the relationship between current and expected prices is \emph{intertemporal arbitrage}.
...
\item
  ...
  \item
    Suppose the market was in equilibrium in period \(t-1\). However, unfavorable weather conditions in period \(t\) shifted the supply curve upward so that: \(Q_t^{0} = -15+0.5P_{t-1}\). Assuming no other shocks to the system in any subsequent periods, obtain the market prices for periods \(t\), \(t+1\), and \(t+2\).
  \end{enumerate}
\end{enumerate}
...
\protect\hyperlink{ref-tomek2014}{Tomek and Kaiser} (\protect\hyperlink{ref-tomek2014}{2014}, Chapters 12 \& 13)
...
Because spot and futures prices are determined simultaneously (and are influenced by the same set of fundamentals), they are correlated. But because holding the phisical commodity is costly, while holding the futures contracts is not, the spot and futures prices typically differ. This difference is referred to as the price of storage, which is the expected return from holding inventory and hedging. The relationship between the price of storage and the amount of inventory held is illustrated by an upward-sloping supply-of-storage function.
...
\item
  current prices reflect all the available information;
\item
  new information (which occurs randomly) is reflected instantly (and correctly) onto prices.
\end{itemize}

Empirically, the efficient markets support the so-called random walk model, given by: \[p_{f,t+1} = p_{f,t} + \varepsilon_{t+1},\;~~\varepsilon_{t+1}\sim iid\left(0,\sigma^2\right).\] Thus, under the efficient market hypothesis, the best predictor of the future price is the present price (because the error term, which depicts information shocks, is mean-zero).

If markets are inefficient, i.e., if price changes do not fully reflect information changes, then past prices can facilitate economically meaningful forecasts: \[p_{f,t+1} = \beta_1 p_{f,t} + \beta_2 p_{f,t-1} + \ldots + \varepsilon_{t+1}.\] In other words, there can be a model, e.g.~an autoregressive model, that can yield forecasts that are, on average, more accurate than those from the random walk model. This can happen, because not everyone has perfect information (semi-strong-form efficient vs.~strong-form efficient), and moreover, even with perfect information, \emph{bounded rationality} may result in sub-optimal decisions by market participants.

\hypertarget{agricultural-policy}{%
\chapter*{Agricultural Policy}\label{agricultural-policy}}
\addcontentsline{toc}{chapter}{Agricultural Policy}
...
In low and middle income countries, barriers to trade are imposed in attempts to distribute benefits to particular groups, to generate government revenues, and to offset economic instability and food insecurity. In addition to these self-imposed restrictions, low and middle income countries can also be affected by externally imposed protectionist policies of the high income countries.

Common levers for a trade restriction are trade quotas, as well as import tariffs and export taxes. The quotas, such as export restrictions, tend to be imposed by some countries somewhat intermittently, during the years with a bad harvest season, or during periods of abnormally high prices for food commodities, for example. Tariffs and taxes are considered less distorting than quotas, and thus are more common longer-term application to trade restrictions. Other government interventions that can still impact/distort trade, albeit to a lesser extent, are various taxes and subsidies, such as, for example, income taxes, value-added taxes, production subsidies, etc.

\hypertarget{the-terms-of-trade}{%
\subsection{The Terms of Trade}\label{the-terms-of-trade}}

The \emph{terms of trade} denotes export prices in terms of import prices. A large country can alter terms of trade in their favor by means of trade restrictions (e.g., tariffs or quotas) or other equivalent trade distortion mechanisms. The welfare effect of this is nonlinear, however. That is, a small tariff may lead to an increase in national welfare, but at some tariff rate, the welfare will begin to decrease as the economic efficiency loss exceeds the terms of trade gain. Likewise, an export tax that completely prohibits exports leaves a country worse off; but an export tax rate may exist that maximizes national welfare through the terms of trade. This, of course, ignores the possibility that other countries may retaliate against the large country by enacting their own trade restrictions.

\hypertarget{the-general-agreement-on-tariffs-and-trade}{%
\section{The General Agreement on Tariffs and Trade}\label{the-general-agreement-on-tariffs-and-trade}}
...
The early rounds primarily focused on negotiations on tariffs and on rules for trading blocs; the middle rounds increasingly focused on non-tariff issues. Agricultural trade restrictions received relatively little attention until the \emph{Uruguay Round}, which commenced in 1986. Prior to this, high income countries strongly resisted compromises on agriculture. Subsequently, a compromise was reached by guaranteeing farmers continued support, and allowing subsidies, that cause not more than minimal trade distortion, to meet public policy aims. Agricultural trade restrictions were at the heart of the \emph{Doha Round}, also called the \emph{Development Round}, under the WTO.

\hypertarget{agriculture-in-the-trade-negotiations}{%
\section{Agriculture in the Trade Negotiations}\label{agriculture-in-the-trade-negotiations}}
...
\tightlist
\item
  Market access - non-tariff barriers to be converted into tariff equivalents; where necessary, minimum access to domestic markets to be guaranteed through the creation of tariff-rate quotas; provisions to be introduced to safeguard importers against import surges and low world prices.
\item
  Export competition - export subsidies to be capped and then reduced in both value and volume.
\item
  Domestic support - the degree of the sector-wide support---better known as the aggregate measurement of support--- to be reduced over time (more sharply and swiftly for high income countries than for low and middle income countries).
\end{itemize}

\hypertarget{doha-round}{%
\subsection{Doha Round}\label{doha-round}}
